...
/*
This test performs a raw byte-based parse of the dcodSchema []byte var.
*/
func TestParseRaw(t *testing.T) {
	var s Schema = NewSchema()
	if err := s.ParseRaw(dcodSchema); err != nil {
...
/*
dcodSchema contains definitions per draft-coretta-oiddir-schema.

This is an UNOFFICIAL Internet-Draft and should not be leveraged
by end users in production scenarios. It is imported here because
it makes for an excellent test candidate, and includes definition
types that rarely appear in the wild (namely nameForms).

Some dITContentRule and dITStructureRule instances have been added
to this variable that DO NOT APPEAR in the original draft. This is
only because such definition types are better suited for custom
tailoring rather than static adoption en masse. Furthermore, they
are needed to provide suitable test coverage.

You can learn more about this draft at https://oid.directory or
at the IETF's DataTracker site.
*/
